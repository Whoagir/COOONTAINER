...
`Types` (`core/types.gd`) — enum'ы: `SizeClass, Facet, LiquidId, Integrity, LotKind, InfoMode, PacingTag, VendorType, Phobia, WorldMode, District, PoliceTrigger` (`PROPERTY_THEFT` — кража с чужого двора) + слои `L_WORLD, L_PLAYER, L_ITEM, L_SHARD, L_VEHICLE, L_NPC, L_LIQUID, L_TRIGGER, L_RAGDOLL`.
...
Состояния: `integrity (Types.Integrity)`, `dirt 0..1`, `paint_color`, `wet`, `taped`, `boarded`, `locked`, `lit`, `burnt`, `liquid_left`, `is_open`, `open_drawers`, `torn`, `glued`, `lot_id`, `stolen`.
...
`Hands`: `held[0..1]`, `active_hand` (0 правая / 1 левая, `swap_hand`=Q), `arm_len` (`arm_out`/`arm_in` = колесо ↑/↓ — разогнуть/согнуть руку), `any_held()`, `holds_tag(tag)`, `host_grab(b, hand)`, `host_release_body(b, throw)`, `host_release_all()`. Мышь ведёт активную ладонь на `arm_len` от плеча; вещь следует В ладонь; `REACH`≈1.45 — высоко не достать без лестницы. При хвате вещь и игрок — `collision_exception` друг с другом (притянул к груди — не выбивает капсулу). Двуручный хват: обе ладони за одной точкой взгляда. Присед: `crouch`=Ctrl, бег=`sprint`=Shift.
...
- `DayNight`: ночь короткая (`NIGHT_FRAC`), солнце дольше висит низко (`SUN_SHAPE`), время на золотом часу течёт медленнее (`GOLD_SLOW`); `golden()` 0..1 — используют небо/туман/ambient/облака (`Clouds` под World). Новая игра стартует в `DEFAULT_TIME` (ранний вечер). Ориентиры: полдень 0.5, золотой час ≈ 0.79, ночь > 0.89. Полный круг ≈ **24 мин** реального времени (`CYCLE_SECONDS=1440`).
- `Sleep`: E на `Bed0..3` ночью / поздним вечером → fade, `time_of_day = 0.28` (утро), хил; днём кровать намекает «приходи ночью».
- `Traffic`: 2–4 косметических машины по `Roads.segments` (без `Vehicles.spawn` / физики), объезд реальных тачек.
- `BuildingDoors`: E на дверях соседских трейлеров/сараев → fade + карманный интерьер; выход той же дверью.
...
- **YardZones** — дворы трейлер-парка: своя зона (бирюзовая рамка «СВОЁ») и чужие у Neighbor* (ржавая «ЧУЖОЕ» + ~6 `ItemBody` с `meta neighbor`). Хост при grab: если чужая зона / neighbor / кража у хантера → `ItemBody.stolen` в `state_dict` (`st`), тост, через ~7 с `PoliceTrigger.PROPERTY_THEFT`. Обычный скупщик отказывается и зовёт ментов; `vendor_dark` (`buys_illegal`) берёт с надбавкой. Свой двор / `meta home` / `street` / активный clear-out lot — легально. Строки: `strings_yard.csv`.
...
- **Ladders** — `$Systems/Ladders`: E залезть/слезть, W/S вверх-вниз, Jump спрыгнуть. Постоянные у полки в трейлере, ToolShelf и у Lot0–2 (hangar/storage/garages/port); на ClearOut — временная в ячейке. `Player.on_ladder`.

...
- Ревью: `godot --path . -- --artshot [--artdir=x] [--time=t]` → `user://shots/[x/]art_*.png` (трейлер-парк, пикап, хантеры в лицо, витрина геройских вещей `ArtShot.HERO_IDS`, игрок 3P, торговец, город).
